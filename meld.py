...
## !!!!! client request should be {"action": "meld", "roomId": 123, "userId": 123, "position": "A"}
#response body ot others: {"action": "meld", "roomId": 123, "userId": 123, "position": "A"}
...
def lambda_handler(event, context):
    ...
    try:
        ...
        update_room_table(roomId, position)
        update_user_table(userId)
        
        #forward to other users in this room
        userIDList=[]
        for p in ['A', 'B', 'C', 'D']:
            if p!=position:
...
def update_room_table(roomId, position):
    roomTable.update_item(
        ...
        UpdateExpression="set user"+position+"Meld = True",
    )
    
def update_user_table(userId):
    userTable.update_item(
        Key={
            'id': userId
        },
        UpdateExpression="set winCount = winCount + :val",
